...
- 修复：按 QQ 空间官方表情资源路径识别普通表情、推荐表情和商城表情，正文及点赞人昵称中的表情资源不再误入说说图片列表。
- 重构：Page 说说读取统一输出图片、视频、音频和普通附件媒体对象，同时保留旧版 `images` 字段兼容既有页面与调用方。
- 新增：读取 `pic[].video_info` 中的真实 QQ 空间视频播放地址和封面，Page 直接在线加载并以 `preload=metadata` 播放预览，不预先下载或落盘。
- 新增：音频使用浏览器原生播放器，其他 QQ 空间附件使用通用文件卡片展示；未知文件类型仍可通过通用附件方式访问。
- 新增：图片、视频、音频和普通附件均提供“下载原文件”入口，直接使用 QQ 返回的原始媒体地址，不转码、不压缩、不经过插件中转。
- 测试：增加表情误判、视频真实字段、音频、普通文件、Page 媒体输出与前端预览/下载回归，并完成全量测试。

## v0.8.2 - 2026-07-13

- 修复：自动发布的随机偏移改为基准时间后的随机延迟，不再因负偏移落到当前时间之前而被钳成 1 秒后执行，避免同一 cron 时段连续发布多条说说。
- 修复：全部 8 个 `qzone_*` LLM 工具统一返回文本给 Agent，由 LLM 组织最终回复，不再把工具结果直接发送给用户或让 Agent 收到空结果。
- 安全：daemon Secret 使用 `hmac.compare_digest` 做常量时间比较，消除字符串短路比较带来的时序侧信道。
- 修复：自动评论无法可靠取得当前登录 QQ 时会中止本轮，避免 `login_uin=0` 导致“跳过自己”保护失效。
- 修复：`/qzone bind` 与 `/qzone autobind` 成功后若状态刷新失败，会明确提示绑定已完成但刷新失败，不再展示陈旧快照。
- 文档：明确随机偏移范围为 0–3600 秒，且语义为基准时间之后的随机延迟。

## v0.8.1 - 2026-06-18

- 优化：Pages 详情查看改为缓存优先、同一说说请求合并与短时后端详情缓存，重复点击带图说说不再反复刷新详情接口。
- 修复：桌面端打开详情不再调用滚动定位，移动端抽屉打开时锁定背景滚动，并保留详情滚动位置，避免带图详情加载后页面位置轻微偏移。
- 优化：WebUI 视觉升级为暖色玻璃卡片、柔和入场动画、稳定滚动条占位、单图详情固定比例占位和发布字数提示，降低图片加载造成的布局跳动。
- 性能：Pages 动态卡片启用内容可见性优化，详情图片渲染增加布局/绘制隔离，后端详情接口 15 秒内复用成功结果。
- 修复：WebUI 查看说说详情时会对详情接口图片和缓存列表图片做双层去重，避免同一张 QQ 空间图片以 `psc`、http/https、尺寸参数等不同 URL 形态返回后刷新出重复照片。
