...
## 2026-07-13 表情误判与多媒体读取

### 正文或点赞人昵称中的表情被当成说说图片

- 症状：Page 动态卡片额外显示 QQ 表情小图，点赞人昵称含表情时也会把表情加载到说说图片区域。
- 根因：旧解析器会递归扫描 HTML 和附件容器中的所有 `<img>`/URL，没有区分 QQ 官方表情、头像、真实照片和其他附件。
- 修复：把图片 URL 规范化集中到单一入口，并按 QQ 空间表情库路径排除普通表情、推荐表情和商城表情；视频、音频和普通文件也不再进入图片集合。
- 回归：覆盖正文表情、点赞人昵称表情、视频封面、视频播放地址、音频、PDF/ZIP 等普通附件和未知附件。

### 视频说说只显示封面且无法播放

- 症状：真实视频说说的封面被当成普通图片，Page 没有得到视频播放 URL。
- 根因：QQ 返回的视频位于 `pic[].video_info.url3`，旧路径只读取 `pic[].url*`。
- 修复：统一媒体解析器读取 `video_info` 的线上播放地址、封面、视频 ID 和时长；Page 使用浏览器原生播放器并只预加载元数据。
- 回归：使用仓库内真实 QQ 空间详情样本确认同一说说规范为一个视频和一张独立原图，视频封面不再混入图片列表。
...
## 2026-07-13 视频播放与下载 500

- 症状：视频卡片无法播放；点击“下载原文件”时 AstrBot 返回 HTTP 500。
- 根因：视频数据曾错误优先使用封面地址；下载路由又把异步迭代器直接放进 Quart 的三元组返回值，真实 WebUI 无法把它转换为流式响应。
- 修复：视频元素固定使用原始视频源，封面只用于 `poster`；下载路由显式构造 Quart `Response`，保留异步流、Range、原始 MIME、文件名和附件响应头。
- 真实验证：QQ 视频源在 Edge 中达到 `readyState=4`，H.264 High + AAC LC 可播放；AstrBot 下载接口返回原始图片字节和 `Content-Disposition: attachment`，不再返回 500。
