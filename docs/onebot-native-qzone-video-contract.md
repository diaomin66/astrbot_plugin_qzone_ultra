...
Daemon-side Web/H5 publishing has been re-enabled as a two-step verified path: `FileBatchControl`/`FileUploadVideo` and cover upload create the video material, `emotion_cgi_publish_v6` creates or reveals the video mood (which may initially be self-visible), and `emotion_cgi_update` is then called with `tid=<fid>`, `ugc_right=1`, `who=1`, `to_tweet=0`, `to_sign=0`, and the video `richval` to switch the mood to public. The daemon still refuses success until feed/detail verification proves `appid=311`, the same `sVid`, the login user as host, and an explicit public marker.
