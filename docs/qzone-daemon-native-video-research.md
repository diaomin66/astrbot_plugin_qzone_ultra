...
日期：2026-06-02；H5 路径更新：2026-06-03；OneBot/Tencent upload 稳定化：2026-06-05；Android has_video/封面业务体对齐：2026-06-05；OneBot 协议端通用化：2026-06-05；Cookie H5 就绪语义更新：2026-06-06；公开视频权限确认：2026-06-06

## 结论

当前仓库里的 daemon 已完全废除 QQ/QQNT 客户端 handoff，也不再把视频帧伪装成图片说说发布。`native_video_publish` 开启时，单个本地视频默认可直接复用已绑定的 Qzone Web Cookie/`p_skey` 走 H5 `video_qzone` 视频上传 + `pic_qzone` 封面上传 + richval 发布；OneBot 未返回 QQ upload A2/vLoginData 只影响 Tencent upload 后备链路，不再阻塞 Cookie H5 后台直发。任一路径上传后都必须在最近动态或详情中验证到 `appid=311` 且同一 `sVid`，否则直接报错，不会把 H5 upload `sVid`、`publish_v6` 的 `tid`、或 `appid=4` 相册/封面动态当作视频说说发布成功。
...
- 如果视频源可读取，daemon 先本地化视频；单个本地视频优先进入 daemon 后台直发链路，优先使用 Web Cookie H5 `video_qzone` + `pic_qzone`，Tencent upload A2/vLoginData 作为后备，渲染结果仍可使用视频封面图保留播放标识。
- 运行时已废除 `mqqapi://qzone/publish` / QQ/QQNT 客户端确认发布路径；客户端跳转只保留在逆向背景说明中，不再由插件调用。
- daemon 原生视频发布在已有 Qzone Web Cookie/`p_skey` 时即可进入 H5 后台直发；未提供 Cookie、视频组合不适合原生发布或关闭 `native_video_publish` 时都会阻止视频发布并提示绑定/调整媒体，不再把视频封面帧或渲染图当作图片说说发出。
- `/qzone autovideoauth` 面向 OneBot 协议端做通用扩展 action 探测：优先确保 daemon 有可用 Qzone Cookie/`p_skey`，再尝试 `get_qzone_video_upload_credentials` / `get_video_upload_credentials` 等协议端自定义 action，其次尝试 `get_login_misc_data(key/name/field=a2/vLoginData/...)`，再尝试 LLOneBot `llonebot_debug` 的登录 misc/A2 入口；`get_cookies`、`get_credentials`、`get_clientkey`、`forceFetchClientKey` 返回的 Web Cookie/CSRF/clientkey/keyIndex 只记录诊断，不会冒充 A2，也不会让 Cookie H5 已就绪状态显示成失败。
...
发布说说时调用 `emotion_cgi_publish_v6`，携带 `richtype=3`、本地上传视频 `subrichtype=6`、上述 `richval`、正文、外层公开视频权限 `who=1`/`ugc_right=1` 等字段。`richval` 内的 `who=5` 是 Qzone 视频链接格式的一部分，不能改成 `1`，否则 Qzone 会拒绝为无效链接；可见范围由发布表单外层字段控制。当前 daemon 在 Qzone Web Cookie/`p_skey` 可用时会把这条 H5 video+cover 路径视为可发布状态，然后仍然轮询最近动态/详情验证 `appid=311` 且同一 `sVid`；只有验证到 feed 且未出现仅自己可见、好友可见、部分可见或非 `ugc_right=1` 的权限标记时才返回成功。A2/vLoginData 驱动的 Tencent upload SDK 链路保留为后备，不再是 Cookie 用户的硬性前置条件。

## v0.6.28 状态语义：Cookie H5 ready，不再误报 A2 缺失失败

- daemon `/health` 的 `video_upload` 摘要新增 `qq_upload_configured`、`web_cookie_configured`、`ready`、`verification_required`。当 Cookie H5 路径可用但 A2 未绑定时，`ready=True`、`web_cookie_configured=True`、`qq_upload_configured=False`、`verification_required=True`。
- `/qzone status` 和 `/qzone autovideoauth` 不再把“OneBot 未返回 QQ upload A2/vLoginData”呈现为视频发布失败；该信息只作为 Tencent upload 后备诊断。真正的成功条件仍是 daemon 发布后验证到 `appid=311` 且同一 `sVid`。
