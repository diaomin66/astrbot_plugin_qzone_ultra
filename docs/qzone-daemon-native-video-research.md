...
当前仓库里的 daemon 已完全废除 QQ/QQNT 客户端 handoff，也不再把视频帧伪装成图片说说发布。`native_video_publish` 开启时，单个本地视频默认稳定路径必须拿到 QQ upload A2/vLoginData，并走 Tencent upload/Android 同源的 `video_qzone` 视频上传 + `pic_qzone` 封面上传链路。仅有 Qzone Web Cookie/`p_skey` 时，H5 `video_qzone` + `pic_qzone` 只能证明上传接口可访问；真实环境已观察到 `emotion_cgi_publish_v6`/richval 会报“链接无效”且产生非公开视频副作用，因此默认不再尝试 H5 发布，除非显式设置实验开关。任一路径上传后都必须在最近动态或详情中验证到 `appid=311`、同一 `sVid`，且未出现仅自己可见、保密、好友可见、部分可见或其它非 `ugc_right=1` 权限标记，否则直接报错，不会把 H5 upload `sVid`、`publish_v6` 的 `tid`/报错、或 `appid=4` 相册/封面动态当作视频说说发布成功。
...
发布说说时可尝试调用 `emotion_cgi_publish_v6`，携带 `richtype=3`、本地上传视频 `subrichtype=6`、上述 `richval`、正文、外层公开视频权限 `who=1`/`ugc_right=1` 等字段。`richval` 内的 `who=5` 是 Qzone 视频链接格式的一部分，不能改成 `1`；真实环境已观察到该接口会对本地上传 `sVid` 返回“您输入的链接不是有效链接”，但封面/上传侧仍可能已经产生动态副作用，因此它不能作为最终失败或成功的来源。当前 daemon 只有在显式设置 `QZONE_ALLOW_EXPERIMENTAL_H5_VIDEO_PUBLISH=1` 时才会尝试 H5 video+cover 发布；无论 `publish_v6` 成功、返回 `tid`、返回 `feedinfo`，还是抛出“链接无效”，都继续轮询最近动态/详情验证 `appid=311` 且同一 `sVid`；只有验证到 feed 且未出现仅自己可见、保密、无访问权限、好友可见、部分可见或非 `ugc_right=1` 的权限标记时才返回成功。默认公开视频稳定路径仍是 A2/vLoginData 驱动的 Tencent upload SDK 链路。
...
- `/qzone status` 和 `/qzone autovideoauth` 会把“OneBot 未返回 QQ upload A2/vLoginData”呈现为公开视频发布未就绪；Cookie/H5 只作为诊断材料，除非显式启用实验 H5 发布。真正的成功条件仍是 daemon 发布后验证到 `appid=311` 且同一 `sVid` 且全部人可见。
