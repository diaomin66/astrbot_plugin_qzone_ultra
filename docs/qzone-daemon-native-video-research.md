...
The H5 path is now an active daemon publish route when Cookie/`p_skey` is available. After `FileUploadVideo` returns `sVid` and the cover upload completes, the daemon calls `emotion_cgi_publish_v6` with `richtype=3`, `subrichtype=7`, and the Web video `richval`. Because this may create a self-visible mood or return a recoverable “invalid link” error while still leaving a video mood behind, the daemon extracts or discovers `tid/fid`, calls `emotion_cgi_update` with `ugc_right=1`/`who=1`/`tid=<fid>` to make it public, then performs final public feed/detail verification before reporting success.
