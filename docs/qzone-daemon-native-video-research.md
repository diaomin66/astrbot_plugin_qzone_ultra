...
日期：2026-06-02；H5 路径更新：2026-06-03；OneBot/Tencent upload 稳定化：2026-06-05；Android has_video/封面业务体对齐：2026-06-05；OneBot 协议端通用化：2026-06-05；Cookie H5 就绪语义更新：2026-06-06；公开视频权限确认：2026-06-06；H5 乱报错与私密权限诊断：2026-06-06

## 结论

Current daemon path no longer uses QQ/QQNT handoff or video-frame image fallback. With `native_video_publish` enabled, single local video publishing requires QQ upload A2/vLoginData and uses the Android-style `video_qzone` + `pic_qzone` chain. Qzone Web Cookie/`p_skey` H5 upload only proves diagnostic reachability; it is not used for publishing because live probes showed invalid-link and non-public side effects. Every success still requires feed/detail verification of `appid=311`, the same `sVid`, and public/all-visible permissions.
...
这条图文链路没有接收本地视频文件、视频分片、`vid` 或腾讯上传 SDK 结果的参数。把本地视频路径拼进正文只会变成普通文本，不能让 QQ 空间上传视频。历史/实测 Web 视频模型曾表现为先走 H5 `sliceUpload/FileUploadVideo` 拿到 `sVid`，再拼视频 `richval`；该路径现在只保留为诊断历史，不作为稳定公开视频发布方案。
...
- 如果视频源可读取，daemon 先本地化视频；单个本地视频优先进入 daemon 后台直发链路，默认必须使用 QQ upload A2/vLoginData 驱动的 Tencent upload/Android 同源路径，渲染结果仍可使用视频封面图保留播放标识。
- 运行时已废除 `mqqapi://qzone/publish` / QQ/QQNT 客户端确认发布路径；客户端跳转只保留在逆向背景说明中，不再由插件调用。
- daemon 原生视频发布缺少 QQ upload A2/vLoginData 时会直接阻止发布；已有 Qzone Web Cookie/`p_skey` 只用于 Cookie 诊断和 H5 实验路径可访问性判断，不再让状态显示为默认可发布。关闭 `native_video_publish`、视频组合不适合原生发布或缺少 A2 时，都会阻止视频发布并提示绑定/调整媒体，不再把视频封面帧或渲染图当作图片说说发出。
...
## v0.6.9 H5 诊断历史：`p_skey` 视频上传 + Web richval 发布尝试
...
H5 `emotion_cgi_publish_v6` / `FileUploadVideo` research is retained only as diagnostic history. The daemon no longer attempts H5 video+cover publishing, because live probes showed invalid-link and non-public side effects. The stable public-video path is the A2/vLoginData Tencent upload SDK chain, or a protocol-end native `publish_qzone_video_mood` action that returns `sVid` and is then verified by the daemon.
...
## v0.6.28 状态语义：Cookie H5 upload-only，不再误报视频发布 ready

- daemon `/health` 的 `video_upload` 摘要新增 `qq_upload_configured`、`web_cookie_configured`、`ready`、`verification_required`。当 Cookie H5 上传接口可用但 A2 未绑定时，`ready=False`、`web_cookie_configured=True`、`qq_upload_configured=False`、`requires=qq_upload_a2_vlogin_data`。
- `/qzone status` and `/qzone autovideoauth` report video publishing as not ready when OneBot does not return QQ upload A2/vLoginData. Cookie/H5 material is diagnostic-only and no longer participates in publishing. Real success still requires daemon verification of `appid=311`, the same `sVid`, and public/all-visible permissions.
...
- The more feasible stable boundary is now protocol-end native publishing before A2 extraction. NapCat/LLBot-style ends can keep QQ upload login material inside their own NTQQ process and expose a OneBot extension action such as `publish_qzone_video_mood`, `publish_qzone_video_shuoshuo`, `qzone_publish_video`, `upload_qzone_video`, or the matching `_...` names.
...
- H5 Cookie `sliceUpload` stays diagnostic/experimental. It is still useful proof that OneBot can obtain Qzone Web credentials, but it is not promoted to default publish because prior live tests showed invalid-link/private-visibility side effects.
...
- The daemon-side publish path remains singular: `video_qzone` video upload plus `pic_qzone` cover upload, with `publishmood` business data embedded and a final feed/detail verification for `appid=311`, same `sVid`, and public/all-visible visibility.
