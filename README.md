...
- 支持文本、图片和视频引用形式的说说发布；单个本地视频会优先交给本地 daemon 通过 Qzone H5 `sliceUpload` + Web 视频说说发布接口直发，已有 Qzone Cookie/`p_skey` 即可尝试，不再依赖 QQ/QQNT 客户端 handoff，也不再把视频帧当图片发出。
...
`native_video_publish` 开启后，单个本地视频会优先走 daemon 原生视频后台路径：daemon 会优先使用已绑定 Qzone Cookie 里的 `p_skey` 调用 H5 `sliceUpload/FileUploadVideo` 上传视频，取得 `sVid` 后再按 Web 模块的 `richval` 规则调用 `emotion_cgi_publish_v6` 发布 `richtype=3`、`subrichtype=7` 的真实视频说说。daemon 之后会轮询最近动态并验证同一 `sVid`，只有验证到真实视频动态才返回发布成功；如果 Cookie 缺少 `p_skey`，则后备尝试 `/qzone videoauth`、`/qzone autovideoauth` 或进程环境变量 `QZONE_VIDEO_UPLOAD_LOGIN_DATA_B64` 提供的 QQ upload A2/vLoginData 材料走旧 Tencent upload SDK 路径。任一路径未验证到 feed 都会阻止发布并报错，不再回退为视频封面图，也不再唤起 QQ/QQNT 客户端确认窗口。关闭 `native_video_publish` 后才会明确按视频封面图发布。
