...
| 发日常说说全自动完整发布 | - | 管理员 | `发日常说说全自动完整发布` | 立即执行 Life Scheduler 日程、LLM 自拍提示词、OmniDraw 自拍图、QQ 空间发布完整链路 |
...
- 管理员可发送 `发日常说说全自动完整发布` 立即跑完整链路并直接发布；该指令会沿用各项日程/LLM/OmniDraw 开关，但会强制发布，不受 `life_publish.mode=draft` 影响。
