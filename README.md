...
- Text/image/video publishing: a single local video is sent to the local daemon. Video publish now requires Qzone Web Cookie/`p_skey` and the H5 `video_qzone` + `pic_qzone` flow: create an only-self-visible video mood, call the permission update API to make it public, then verify it. It no longer uses QQ/QQNT handoff, OneBot protocol-end publish, QQ upload A2/vLoginData, or video-cover image fallback.
...
`native_video_publish` enabled means a single local video is published through a verified daemon path. With a usable Qzone Web Cookie/`p_skey`, the daemon uses the H5 `sliceUpload/FileUploadVideo` + cover upload path, calls `emotion_cgi_publish_v6` to create an only-self-visible video mood, discovers/keeps the resulting `tid/fid`, then calls `emotion_cgi_update` with `ugc_right=1`/`who=1` to make it public. Success is reported only after daemon verification finds `appid=311`, the same `sVid`, and explicit public/all-visible feed/detail; no cover-image, OneBot protocol-end publish, Tencent upload, or QQ/QQNT UI fallback is used.

`/qzone autovideoauth` now only ensures the daemon has usable Qzone Cookie/`p_skey` for the H5 video upload, private-create, permission-update, and public-verification chain; it no longer probes or binds QQ upload A2/vLoginData.
When status shows `video_upload: ready`, the Cookie/H5 video path is available. The daemon uploads the video and cover, publishes the video mood as only-self-visible first, calls the permission-update CGI to switch it to public, and only reports success after public feed/detail verification.
  ...
  - Video publish contract: all successful video posts must come from the daemon H5 private-create + permission-update + public-verification chain. If Cookie/`p_skey` is missing, permission update fails, or public feed/detail verification fails, the plugin refuses success instead of duplicating the post or falling back to a cover image.
