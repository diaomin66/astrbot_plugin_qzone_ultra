...
- 支持文本、图片和视频引用形式的说说发布；单个本地视频会优先交给本地 daemon 通过 Tencent upload 后台直发，未获得 QQ upload 登录材料时会阻止发布并提示绑定材料，不再把视频帧当图片发出，也不再打开 QQ/QQNT 客户端。
...
`native_video_publish` 开启后，单个本地视频会优先走 daemon 原生视频后台路径：如果 daemon 已通过 `/qzone videoauth`、`/qzone autovideoauth` 或进程环境变量 `QZONE_VIDEO_UPLOAD_LOGIN_DATA_B64`（可选 `QZONE_VIDEO_UPLOAD_LOGIN_KEY_B64`、`QZONE_VIDEO_UPLOAD_TOKEN_*`）获得 QQ upload 二进制登录材料，就会按 Android 客户端链路先用 Tencent upload SDK 的 `video_qzone` 协议提交视频，并把 `publishmood` 业务体嵌入 `UploadVideoInfoReq.vBusiNessData`；拿到 `sVid` 后再用 `pic_qzone` 上传视频封面，封面请求携带 `vid`、`clientkey`、`mobile_fakefeeds_clientkey` 与 `mix_*` 字段触发视频动态生成。daemon 之后会轮询最近动态并验证同一 `sVid`，只有验证到真实视频动态才返回发布成功；如果未获得材料、只获得 `clientKey/p_skey`，或上传后未验证到 feed，都会阻止发布并报错，不再回退为视频封面图，也不再唤起 QQ/QQNT 客户端确认窗口。PC Cookie、`p_skey`、`clientKey` 不能直接等价为这份二进制上传登录材料。关闭 `native_video_publish` 后才会明确按视频封面图发布。
