...
- 支持文本、图片和视频引用形式的说说发布；单个本地视频会交给本地 daemon 后台直发。默认稳定公开视频路径需要 OneBot 协议端或手动配置提供 QQ upload A2/vLoginData；仅有 Qzone Web Cookie/`p_skey` 时，H5 `video_qzone` + `pic_qzone` 只能作为实验诊断路径，不会默认发布，避免生成仅自己可见的视频动态。不再依赖 QQ/QQNT 客户端 handoff，也不再把视频帧当图片发出。
...
`native_video_publish` enabled means a single local video is published through the verified native video path. The stable daemon path requires QQ upload A2/vLoginData from `/qzone videoauth`, `/qzone autovideoauth`, or `QZONE_VIDEO_UPLOAD_LOGIN_DATA_B64`, then uses the Android-style Tencent upload chain `video_qzone` + `pic_qzone` with public visibility embedded in the business payload. Qzone Web Cookie/`p_skey` and H5 `sliceUpload/FileUploadVideo` remain diagnostic-only and are never used for publishing, because live probes can create invalid-link or non-public side effects. Success is reported only after daemon verification finds `appid=311`, the same `sVid`, and public/all-visible feed/detail; no cover-image fallback or QQ/QQNT UI prompt is used.
...
When status shows `video_upload: ready`, QQ upload A2/vLoginData is configured. `web_cookie_configured: True` or `h5_video_upload: diagnostic_only` only means diagnostic material is available, not that video publishing is ready.
  ...
  - Native OneBot video publish contract: if a protocol end exposes `publish_qzone_video_mood`, `publish_qzone_video_shuoshuo`, `qzone_publish_video`, `upload_qzone_video` or the matching `_...` extension actions, the plugin first asks the protocol end to publish the single trusted local video with `who=1`, `ugc_right=1` and public visibility fields. The local daemon then verifies `appid=311`, the same returned `sVid`, and public visibility before reporting success. If the protocol action succeeds without `sVid`, reports private/friend-only visibility, or daemon verification fails, the plugin refuses success instead of duplicating the post or falling back to a cover image. 协议端实现细节见 [`docs/onebot-native-qzone-video-contract.md`](docs/onebot-native-qzone-video-contract.md)。
