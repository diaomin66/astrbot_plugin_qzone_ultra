...
- 支持文本、图片和视频引用形式的说说发布；单个本地视频会优先交给本地 daemon 通过 Tencent upload 后台直发，未获得 QQ upload 登录材料时只提取封面按图片发布，不再打开 QQ/QQNT 客户端。
...
| 发说说 | - | 管理员 | `发说说 <文本> [图片/视频]` | 立即发布说说；单视频优先走 daemon 原生视频后台直发，缺少上传材料时以封面图发布 |
...
`native_video_publish` 开启后，单个本地视频会优先走 daemon 原生视频后台路径：如果 daemon 已通过 `/qzone videoauth`、`/qzone autovideoauth` 或进程环境变量 `QZONE_VIDEO_UPLOAD_LOGIN_DATA_B64`（可选 `QZONE_VIDEO_UPLOAD_LOGIN_KEY_B64`、`QZONE_VIDEO_UPLOAD_TOKEN_*`）获得 QQ upload 二进制登录材料，就会按 Tencent upload SDK 的 `video_qzone` 协议提交视频，并把 `publishmood` 业务体嵌入 `UploadVideoInfoReq.vBusiNessData` 触发后台发布；未获得材料时只回退为视频封面图发布，不再唤起 QQ/QQNT 客户端确认窗口。PC Cookie 不能直接生成这份二进制上传登录材料。
