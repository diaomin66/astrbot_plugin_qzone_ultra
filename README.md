...
- 支持文本、图片和视频引用形式的说说发布；单个本地视频会优先交给本地 daemon 后台直发：已有 Qzone Web Cookie/`p_skey` 时走 H5 `video_qzone` 视频上传 + `pic_qzone` 视频封面上传 + feed `sVid` 验证路径；若协议端额外提供 vLoginData/A2，则 QQ upload/Tencent upload 作为后备路径。不再依赖 QQ/QQNT 客户端 handoff，也不再把视频帧当图片发出。
...
`native_video_publish` 开启后，单个本地视频会优先走 daemon 原生视频后台路径。默认推荐路径是复用已绑定 Qzone Web Cookie/`p_skey`：先通过 H5 `sliceUpload/FileUploadVideo` 上传视频取得 `sVid`，再通过 H5 `sliceUpload/FileUpload` 以 `pic_qzone` 上传视频封面，并携带 `vid`、`clientkey`、`mobile_fakefeeds_clientkey`、`is_client_upload_cover`、`is_pic_video_mix_feeds`、`mix_videoSize`、`mix_time` 等字段触发真实视频动态；最后轮询最近动态/详情验证同一 `sVid`。`/qzone videoauth`、`/qzone autovideoauth` 或环境变量 `QZONE_VIDEO_UPLOAD_LOGIN_DATA_B64` 绑定的 vLoginData/A2 仍保留为 QQ upload/Tencent upload 后备路径。未验证到 feed 时会阻止发布并报错，不再回退为视频封面图，也不再唤起 QQ/QQNT 客户端确认窗口。关闭 `native_video_publish` 后会直接拒绝包含视频的发布请求，必须重新开启并至少绑定 Qzone Cookie 后才能发布真视频。

`/qzone autovideoauth` 面向 OneBot 协议端能力探测，不绑定单一实现：会优先确保 daemon 已有可用 Qzone Cookie/`p_skey`，再尝试通用扩展 action（例如 `get_qzone_video_upload_credentials`、`get_video_upload_credentials`、`get_login_misc_data key=a2/vLoginData` 等，也兼容 `_get_*` 这类下划线扩展命名），也会兼容 LLOneBot 的 `llonebot_debug` 透传 `nodeIKernelLoginService/getLoginMiscData`、`loginService.getLoginMiscData` 和 PMHQ `httpSend`。调用层支持 `call_action`、`call_api`、`request`、`call` 以及 `params/data/payload` 包装，NapCat / LLOneBot / LLBot / Shamrock 等 OneBot 实现只要暴露返回 vLoginData/A2 二进制材料的 action 都可被绑定；返回字段支持 `a2`/`a2_hex`/`a2_b64`、`vLoginData`/`vLoginDataHex`/`vLoginDataB64`、Node `Buffer`/数字数组，以及目标 login-misc 调用返回的原始二进制字符串。仅返回 Cookie/CSRF 或 `clientkey/keyIndex` 不会被误当成 A2；如果 OneBot 只返回 Cookie/CSRF，命令也会把 Cookie 绑定到 daemon 并启用 H5 video+cover 后台直发路径。
当状态里显示 `video_upload: ready` 且 `web_cookie_configured: True` 时，表示 Web Cookie H5 后台直发已可用；`qq_upload_configured: False` 只代表 Tencent upload 后备 A2 未绑定，不是视频发布不可用。
