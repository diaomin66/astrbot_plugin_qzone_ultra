...
- 支持文本、图片和视频引用形式的说说发布；单个本地视频会交给本地 daemon 后台直发。可用 Qzone Web Cookie/`p_skey` 时优先走 H5 `video_qzone` + `pic_qzone` 上传，先发布为 QQ 空间可接受的视频说说，再调用权限修改接口改成全部人可见并二次校验；也兼容 QQ upload A2/vLoginData 的 Tencent upload 路径和 OneBot 协议端原生发布。不再依赖 QQ/QQNT 客户端 handoff，也不再把视频帧当图片发出。
...
`native_video_publish` enabled means a single local video is published through a verified daemon path. With a usable Qzone Web Cookie/`p_skey`, the daemon now uses the H5 `sliceUpload/FileUploadVideo` + cover upload path, calls `emotion_cgi_publish_v6` to create the video mood, discovers/keeps the resulting `tid/fid`, then calls `emotion_cgi_update` with `ugc_right=1`/`who=1` to make it public. QQ upload A2/vLoginData from `/qzone videoauth`, `/qzone autovideoauth`, or `QZONE_VIDEO_UPLOAD_LOGIN_DATA_B64` remains supported through the Android-style Tencent upload chain. Success is reported only after daemon verification finds `appid=311`, the same `sVid`, and explicit public/all-visible feed/detail; no cover-image fallback or QQ/QQNT UI prompt is used.

`/qzone autovideoauth` 面向 OneBot 协议端能力探测，不绑定单一实现：会优先确保 daemon 已有可用 Qzone Cookie/`p_skey` 作为诊断材料，再尝试通用扩展 action（例如 `get_qzone_video_upload_credentials`、`get_video_upload_credentials`、`get_login_misc_data key=a2/vLoginData` 等，也兼容 `_get_*` 这类下划线扩展命名），也会兼容 LLOneBot/LLBot 风格的 `llonebot_debug` 透传 `nodeIKernelLoginService/getLoginMiscData`、`loginService.getLoginMiscData` 和 PMHQ `httpSend`。调用层支持 `call_action`、`call_api`、`request`、`call` 以及 `params/data/payload` 包装；NapCat / LLOneBot / LLBot / Shamrock 等 OneBot 实现只要暴露返回 vLoginData/A2 二进制材料的 action 都可被绑定。返回字段支持 `a2`/`a2_hex`/`a2_b64`、`vLoginData`/`vLoginDataHex`/`vLoginDataB64`、Node `Buffer`/数字数组，以及目标 login-misc 调用返回的原始二进制字符串。仅返回 Cookie/CSRF、PSKey 或 `clientkey/keyIndex` 不会被误当成 A2；目标 A2 action 返回 `result=-1,value=""` 会被记录为“有接口但没有材料”。
When status shows `video_upload: ready`, either QQ upload A2/vLoginData is configured, or the Cookie/H5 video path is available. For the H5 path the daemon uploads the video and cover, publishes the video mood, calls the permission-update CGI to switch it to public, and only reports success after public feed/detail verification.
  ...
  - Native OneBot video publish contract: if a protocol end exposes `publish_qzone_video_mood` or `_publish_qzone_video_mood`, the plugin asks it once to publish the single trusted local video with `who=1`, `ugc_right=1` and public visibility fields. The protocol response must include success status, `sVid`, and a public marker such as `ugc_right=1`, `visibility=public`, or `public=true`. The local daemon then verifies `appid=311`, the same returned `sVid`, and positive public visibility before reporting success. If the protocol action times out after invocation, succeeds without `sVid`, lacks a public marker, reports private/friend-only visibility, or daemon verification fails, the plugin refuses success instead of duplicating the post or falling back to a cover image. 协议端实现细节见 [`docs/onebot-native-qzone-video-contract.md`](docs/onebot-native-qzone-video-contract.md)。
