...
`native_video_publish` 开启后，单个本地视频会优先走 daemon 原生视频后台路径。默认稳定路径是 `/qzone videoauth`、`/qzone autovideoauth` 或环境变量 `QZONE_VIDEO_UPLOAD_LOGIN_DATA_B64` 绑定 QQ upload A2/vLoginData，然后走 Tencent upload/Android 同源的 `video_qzone` 视频上传和 `pic_qzone` 封面上传链路，并在上传业务体里携带公开视频权限。仅有 Qzone Web Cookie/`p_skey` 时，H5 `sliceUpload/FileUploadVideo` 与 `pic_qzone` 封面上传可用于诊断，但本地实测 `emotion_cgi_publish_v6` 可能报“链接无效”且产生非公开视频副作用；因此默认不会尝试 H5 发布，除非显式设置 `QZONE_ALLOW_EXPERIMENTAL_H5_VIDEO_PUBLISH=1`。未验证到 `appid=311` 且同一 `sVid` 的全部人可见 feed/detail 时会阻止发布并报错，不再回退为视频封面图，也不再唤起 QQ/QQNT 客户端确认窗口。
...
当状态里显示 `video_upload: ready` 时，表示已绑定 QQ upload A2/vLoginData，或显式启用了实验 H5 发布并仍需 feed/detail 公开视频校验；仅 `web_cookie_configured: True` 不再代表视频发布可用。
