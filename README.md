...
QQ空间Ultra 是一个面向 AstrBot 的 QQ 空间插件，提供中文命令、LLM 工具、Cookie 绑定、本地 daemon、图片/视频封面发布、说说渲染、投稿审核和自动评论能力。插件优先适配 OneBot v11 / aiocqhttp，也支持手动 Cookie 绑定后使用核心 QQ 空间功能。
...
- 支持文本、图片和视频引用形式的说说发布；单个本地视频会优先唤起 QQ/QQNT 原生发布窗口，不可用时会提取封面再按图片发布，并可返回带播放标识的 QQ 空间风格发布结果图。
...
| 发说说 | - | 管理员 | `发说说 <文本> [图片/视频]` | 立即发布说说；单视频优先走 QQ 原生发布，失败时以封面图形式发布 |
...
| `native_video_publish` | `true` | 单个本地视频说说优先唤起 QQ/QQNT 原生视频发布窗口；关闭后始终按视频封面图发布 |
...
`native_video_publish` 当前是 QQ 客户端唤起/人工确认路径，不是 daemon 后台真视频直发。daemon 原生视频直发正在按 Tencent upload SDK 逆向推进，已固化 `video_qzone`、`video.upqzfile.com:80`、PDU 帧和 cmd=1/2 协议骨架；真正后台直发还需要补齐 JCE/Tars、QQ upload 二进制登录材料和最终发布 RPC。
